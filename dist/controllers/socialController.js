...
class SocialController {
    ...
    static async createGroup(req, res) {
        try {
            ...
            const group = await database_1.prisma.socialGroup.create({
                data: {
                    name,
                    description,
    ...
    static async joinGroup(req, res) {
        try {
            ...
            const group = await database_1.prisma.socialGroup.findUnique({
                where: { id: groupId }
            });
            if (!group) {
                throw (0, errorHandler_1.createError)('Group not found', 404);
            }
            if (group.isPrivate) {
                throw (0, errorHandler_1.createError)('Cannot join private group', 400);
            }
            await database_1.prisma.socialGroup.update({
                where: { id: groupId },
                data: {
                    members: {
                        connect: {
    ...
    static async createChallenge(req, res) {
        try {
            ...
            const group = await database_1.prisma.socialGroup.findFirst({
                where: {
                    id: groupId,
                    members: {
                        some: {
            ...
            if (!group) {
                throw (0, errorHandler_1.createError)('You are not a member of this group', 403);
            }
            const challenge = await database_1.prisma.groupChallenge.create({
                data: {
                    groupId,
                    title,
                    description,
            ...
            const groupMembers = await database_1.prisma.socialGroup.findUnique({
                where: { id: groupId },
                select: { members: true }
            });
    ...
    static async getGroupChallenges(req, res) {
        try {
            const { groupId } = req.params;
            const { status = 'active' } = req.query;
            const challenges = await database_1.prisma.groupChallenge.findMany({
                where: {
                    groupId,
    ...
    static async updateChallengeProgress(req, res) {
        try {
            ...
            const userChallenge = await database_1.prisma.userChallenge.findFirst({
                where: {
                    challengeId,
                    userId: req.user.id
                }
            ...
            if (!userChallenge) {
                throw (0, errorHandler_1.createError)('Challenge not found or not enrolled', 404);
            }
            const updatedChallenge = await database_1.prisma.userChallenge.update({
    ...
    static async getLeaderboard(req, res) {
        try {
            ...
            const leaderboard = await Promise.all(groupMembers.members.map(async (member) => {
    ...
    static async getUserSocialStats(req, res) {
        try {
            ...
            const [groupsCount, challengesCount, achievementsCount, rank] = await Promise.all([
                database_1.prisma.socialGroup.count({
                    where: {
                        members: {
                            some: { id: userId }
                        }
                ...
                database_1.prisma.userChallenge.count({
                    where: { userId }
                }),
    ...
    static async calculateCarbonImpact(userId) {
