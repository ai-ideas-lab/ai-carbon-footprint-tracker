...
class AIController {
    constructor() {
        if (!AIController.openai) {
            AIController.openai = new openai_1.OpenAI({
                apiKey: process.env.OPENAI_API_KEY
            });
        }
    }
    // Analyze a carbon record with AI
    static async analyzeRecord(req, res) {
        try {
            const { recordData, context } = req.body;
            const { error } = validation_1.aiValidationSchemas.analyzeRecord.validate(req.body);
            if (error) {
...
请用中文回复，使用以下JSON格式：
{
  ...
  "longTermStrategies": ["策略1", "策略2"]
            ...
            try {
                ...
                await database_1.prisma.carbonRecord.update({
                    where: {
                        id: recordData.id
                    },
                    data: {
    ...
    static async generateReductionPlan(req, res) {
        try {
            ...
            if (records.length === 0) {
                throw (0, errorHandler_1.createError)('No carbon data available to generate plan', 400);
            }
...
- 各类别分布：
${Object.entries(byCategory).map(([category, emission]) => `- ${category}: ${emission.toFixed(2)} kg CO2e (${(emission / totalEmission * 100).toFixed(1)}%)`).join('\n')}
...
请用中文回复，使用以下JSON格式：
{
  ...
  "motivation": "激励策略"
            ...
            try {
                ...
                const userPreference = await database_1.prisma.userPreference.findFirst({
                    where: { userId }
                });
                if (userPreference) {
    ...
    static async chatAdvice(req, res) {
        try {
...
请用中文回答，保持专业但友好的语调。`;
            ...
            await database_1.prisma.carbonRecord.create({
                data: {
                    userId: req.user.id,
                    category: 'OTHER',
                    type: 'INDIRECT',
...
${Object.entries(records.reduce((acc, record) => {
                ...
                return acc;
            }, {})).map(([category, count]) => `- ${category}: ${count} 条记录`).join('\n')}
